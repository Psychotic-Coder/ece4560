...
class RedObjectFollower:
    def __init__(self):
        ...
        self.obstacle_threshold = 0.5  # 0.5m obstacle threshold
        self.safety_region = 30  # Pixels around the object to check for obstacles

        # Motion control
        self.current_speed = 0.0
        self.max_speed = 0.5
        self.min_speed = 0.05
        self.acceleration = 0.02  # Rate of smooth acceleration
        self.deceleration = 0.03  # Rate of smooth deceleration
        self.pid_kp = 0.5
        self.pid_kd = 0.1
    ...
    def check_for_obstacles(self, depth_image):
        x, y = self.red_object_center
        region = depth_image[y - self.safety_region:y + self.safety_region,
                             x - self.safety_region:x + self.safety_region]
        
        valid_depths = region[~np.isnan(region)]
        if valid_depths.size > 0 and np.min(valid_depths) < self.obstacle_threshold:
    ...
    def navigate_around_obstacle(self):
        ...
        rospy.loginfo("Obstacle detected, navigating...")

    def move_towards_red_object(self):
        twist = Twist()

        # Dynamic speed adjustment
        target_speed = max(self.min_speed, min(self.max_speed, 0.1 * self.red_object_distance))
        ...
        # PID control for smooth angular movement
        error = (self.red_object_center[0] - self.image_width / 2) / float(self.image_width / 2)
        twist.angular.z = -self.pid_kp * error - self.pid_kd * (error - self.previous_error)
